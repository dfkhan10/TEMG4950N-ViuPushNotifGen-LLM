...
export const Homepage = () => {
  ...
  const fetchViuData = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/viuData`
    );
  ...
  const isFormValid = () => {
    return (
      ...
      (promotionType === "cast-driven"
        ? document.querySelector('input[placeholder="Enter Star Name"]').value
        : true)
    );
  };
      ...
      <div className="flex flex-grow bg-[#F5F5F5]">
        ...
        <div className="flex-grow flex flex-col items-center">
          <div className="flex items-center pr-11 mb-8 mt-6">
            ...
            <h2 className="text-5xl font-bold text-[#F5B919] ml-8">
          ...
          <div className="flex flex-col w-full px-36">
            <div className="flex space-x-10 justify-center">
              ...
              <div className="flex-grow mb-4 w-full">
                <div className="flex">
                  <label className="block text-sm font-bold text-lg text-gray-700">
                    Promotion Type
                  </label>
                  <label className="text-red-400 font-bold text-lg ml-1">
                    *
                  </label>
                </div>
                <select
            ...
            <div className="flex flex-col w-full mb-4">
              <label className="block text-sm font-bold text-lg text-gray-700 mb-2">
                Additional Input
              </label>
              <textarea
            ...
            <div className="flex justify-center mb-4">
              <button
                className={`bg-[#F5B919] w-full text-3xl text-black font-bold py-3 px-6 rounded-lg shadow-md hover:bg-yellow-600 transition duration-300 ${
                  isFormEnabled ? "" : "cursor-not-allowed opacity-50"
                }`}
        ...
        <div className="flex flex-col justify-center items-center w-2/5">
          ...
          <div className="w-4/5 h-5/6 bg-white flex justify-center items-center rounded-lg shadow-md mt-4">
            {!showTrends ? (
              <button
                className={`bg-[#F5B919] text-black font-bold py-2 px-4 hover:bg-yellow-600 rounded-full cursor-not-allowed" ${
                  isFormEnabled ? "" : "cursor-not-allowed opacity-50"
                }`}
                disabled={!isFormEnabled}
              ...
              <div className="flex flex-col w-full">
                <TrendComponent
                  ...
                  description="Kim Na Huels' latest comments at the K-Drama press conference"
                />
                <TrendComponent
                  title="Tropical Trends"
                  description="Residents told to stay home as temperatures set to cross 35 degrees this week"
                />
                <TrendComponent
                  title="Holiday Spirit"
                  description="Public holiday this Saturday on Malaysia Day"
                />
                <TrendComponent
                  title="Pop Culture"
                  description="K-pop group Black Pink is coming to Malaysia for their world tour"
                />
              </div>
            )}
          </div>
